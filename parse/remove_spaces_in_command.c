...
** remove_spaces_in_command.c
** File description:
** remove useless spaces in command
*/
/*
 __  __        _                            ___            ___
|  \/  |      | |                          / _ \          / _ \.
...
bool is_there_no_command(char **word_array)
{
    ...
    int *status)
{
    ...
    if (is_there_no_command(word_array)) {
        if ((sep[0] == '|' && !sep[1]) || pipe_in) {
            *status = 0;
            null_command();
            return NULL;
        }
