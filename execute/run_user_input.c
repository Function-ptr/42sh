...
int process_quote(char **str)
{
    ...
    char *quote_start = double_quote_start == NULL ? simple_quote_start :
    simple_quote_start == NULL ? double_quote_start : double_quote_start <
    simple_quote_start ? double_quote_start : simple_quote_start;
    if (quote_start == NULL) return -1;
    char *quote_end = strchr(quote_start + 1, *quote_start);
    if (quote_end == NULL) return *quote_start;
    ...
    int i = 0; for (; *str + i < quote_start; i++) new_str[i] = (*str)[i];
    for (int j = 0; j < (quote_end - quote_start - 1) * 2; j++, i++)
        new_str[i] = inside_quote_processed[j];
    for (char *c = quote_end + 1; c < *str + str_len; c++, i++) new_str[i] = *c;
    free(inside_quote_processed); *str = new_str; return 0;
}
